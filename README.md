...
> Сборка основана на [Build-R2x1](https://github.com/0x4C334E3438/Build-R2x1) для устройства от LENAR, логика сборки переработана под R281.
...
## О разблокировке LTE-модуля

Штатному LTE-модулю для работы требуется код разблокировки. Известны два способа заставить его работать в OpenWrt:

1. **Получение кода разблокировки из заводской прошивки** и внесение его в скрипт инициализации OpenWrt — описано ниже.
2. **Перепрошивка LTE-модуля** модифицированной прошивкой от LENAR с вырезанной процедурой разблокировки — [способ описан здесь](https://4pda.to/forum/index.php?showtopic=1044372&view=findpost&p=143896115).

> ⚠️ **Важно:** перед прошивкой версии от Megafon в OpenWrt обязательно выполните **постоянную** разблокировку LTE-модуля. Временная разблокировка теряется при перепрошивке!
...
Ниже — сжатая инструкция по разблокировке заводской прошивки C300-1 (полная версия — в `С300-1final.pdf` в репозитории).
...
- ~~Проблемы с установкой пакетов `kmod-...` из-за несовпадения hash ядра.~~ **Исправлено** — логика сборки изменена, `kmod`-пакеты теперь устанавливаются корректно.
